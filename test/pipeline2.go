...
// the second stage demos a transitions from 32bit to 8 bits words along the pipeline 
...
func stage2(pipe1 chan uint32, pipe2 chan byte){
	...
	for (count < total) {
		val = <- pipe1 ;
		byte0 = uint8( (val & 0xFF000000) >> 24);
		byte1 = uint8( (val & 0x00FF0000) >> 16);
		pipe2 <- byte0;
		byte2 = uint8( (val & 0x0000FF00) >> 8) ;
		pipe2 <- byte1;

		byte3 = uint8(val & 0x000000FF);
		pipe2 <- byte2;
		pipe2 <- byte3;
